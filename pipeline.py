...
X = np.zeros((len(PIDS), n_conditions, n_trials))
...
#for p, pid in enumerate([1001]):
    ...
    epo_wm = mne.Epochs(raw_wm, eve_wm, [16384, 32768], -0.5, 1.5,
                        preload=True, #reject=dict(eeg=100e-6),
                        proj=False, add_eeg_ref=False)
    x_pv_neu = epo_pv['16384'].get_data()[:, :72]
    x_pv_neg = epo_pv['32768'].get_data()[:, :72]
    x_wm_neu = epo_wm['16384'].get_data()[:, :72]
    x_wm_neg = epo_wm['32768'].get_data()[:, :72]
    for i in range(n_trials):
        X[p, 0, i] = max(x_pv_neu[i].max(1) - x_pv_neu[i].min(1))
        X[p, 1, i] = max(x_pv_neg[i].max(1) - x_pv_neu[i].min(1))
        X[p, 2, i] = max(x_wm_neu[i].max(1) - x_pv_neu[i].min(1))
        X[p, 3, i] = max(x_wm_neg[i].max(1) - x_pv_neu[i].min(1))
